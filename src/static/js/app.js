...
const Edit = ({ className = '', ...props }) => (
  React.createElement('svg', { className, ...iconProps, ...props },
    React.createElement('path', { key: 'path-1', d: 'm11.498 12.5.032-.024 1.464 1.464L16 10.936l.037.037.465-.465c.59-.59.59-1.546 0-2.136L14.965 6.83a1.513 1.513 0 0 0-2.136 0l-.465.465L12 7.658v.002l.465.465L11 9.59l1.465 1.464-.967.966-.033-.024M13.8 7.9a.3.3 0 0 1 .425 0l1.541 1.542a.3.3 0 0 1 0 .425l-.465.464L13.335 8.4l.465-.5z' }),
    React.createElement('path', { key: 'path-2', d: 'm8.12 12.847-.025-.026-1.465-1.465-.964.964.025.026 1.464 1.465.965-.964z' }),
    React.createElement('path', { key: 'path-3', d: 'M6.75 12h4.95l1.4-1.4-1.6-1.6H7.75a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h4.11l-1.11 1.11-1.11-1.11H6.75z' })
  )
);
...
function App() {
  ...
  const handleManualSummary = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification('每日总结生成成功！', 'success');
  ...
  const getHistoryEntriesByDate = () => {
    ...
    entries.forEach((entry, index) => {
      const entryDate = entry.timestamp ? entry.timestamp.split('T')[0] : null;
      if (entryDate) {
  ...
  const getFilteredEntries = () => {
    let filtered = entries.filter(entry => {
      ...
      // 情绪过滤
      if (filterMood) {
        include = include && entry.ai_analysis && entry.ai_analysis.toLowerCase().includes(filterMood.toLowerCase());
      }
    ...
    React.createElement('main', {
      ...
      React.createElement(TabsContent, {
        ...
        React.createElement('div', {
          ...
          return historyArray.map(({ date, entries: dayEntries }) => {
            // 检查这一天是否有每日总结
            const summaries = dayEntries.filter(e => e.is_daily_summary);
