...
#### 1. 登录与管理员密码
首次启动时系统会要求设置进入应用的登录密码；在首次打开设置页时会要求再设置一份管理员密码。
管理员密码用于修改配置和更改密码等敏感操作。
如果需要通过命令行重置密码，可执行：

```bash
python scripts/reset_password.py --entry NEWPASS --admin NEWADMIN
```
...
### Q: 如何修改密码？
A: 登录后在设置页可以修改登录密码或管理员密码，也可以使用 `scripts/reset_password.py` 脚本在服务器上重置。
