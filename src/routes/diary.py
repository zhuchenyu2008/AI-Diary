...
def get_analysis_status(entry_id):
    ...
    try:
        ...
        # 检查是否有AI分析结果
        has_analysis = entry.ai_analysis is not None and entry.ai_analysis.strip() != ""
        
        return jsonify({
            'success': True,
            'entry_id': entry_id,
            'has_analysis': has_analysis,
            'ai_analysis': entry.ai_analysis if has_analysis else None,
            'status': 'completed' if has_analysis else 'analyzing'
        })
